...
def update_cust(l):
    ...
    if cKey == "1":
        while True:
            ...
            if new_name.isalpha():
                l[idx]["name"] = new_name
                print("[{0}] 번 페이지의 [{1}] 항목이 [{2}] (으)로 변경되었습니다.".format(
                    idx, "name", new_name))
                break
            else:
...
def delete_cust(l, p):
    ...
    if p == -1:
        ...
        while True:
            del_page = int(
                input("삭제할 페이지 번호를 입력해주세요 (0 ~ %d) : " % (len(l)-1)))
            if del_page < 0 or del_page >= len(l):
                print("잘못된 페이지 번호입니다.")
            else:
                break
        while True:
            confirm = input("[%s] 님의 정보를 삭제하시겠습니까? (y/N) : " %
                            l[del_page]["name"]).upper()
            if confirm == "Y":
                del l[del_page]
                print("성공적으로 [%d] 번 페이지를 삭제했습니다." % del_page)
                if del_page >= len(l) - 1:
                    p -= 1
                elif del_page == 0 and len(l) == 0:
                    p = -1
                elif del_page == 0 and len(l) != 0:
                    p -= 1
                break
